...
class Solution {
    ...
    public Node sortedArrayToBST(int[] nums) {
        ...
        formBST(nums, 0, nums.length - 1);
        
        return root;
    }
    
    static void formBST(int[] nums, int low, int high) {
        ...
        if (low <= high) {
            ...
            formBST(nums, low, mid - 1);
            formBST(nums, mid + 1, high);
            
        }
        
    }
    
    static Node insertion(Node root, int [] nums, int pos) {
        
        if (root == null) {
            
            return new Node(nums[pos]);
        }
        
        if (nums[pos] <= root.data) {
            
            root.left = insertion(root.left, nums, pos);
        } else {
            
            root.right = insertion(root.right, nums, pos);
        }
